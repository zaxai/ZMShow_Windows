...
BOOL ZOperateRegedit::SetAutoRun(const CString & strExePath, LPCTSTR lpValueName, BOOL nIsAutoRun)
{
	BOOL nRtn = FALSE;
	HKEY hKey;
...
BOOL ZOperateRegedit::CheckAutoRun(const CString & strExePath, LPCTSTR lpValueName)
{
	BOOL nRtn = FALSE;
	HKEY hKey;
	CString strSubKey(_T("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run"));
...
BOOL ZOperateRegedit::SetREG_SZData(LPCTSTR lpSubKey, LPCTSTR lpValueName, const CString & strData)
{
	BOOL nRtn = FALSE;
	HKEY hKey;
...
BOOL ZOperateRegedit::GetREG_SZData(LPCTSTR lpSubKey, LPCTSTR lpValueName, CString & strData)
{
	BOOL nRtn = FALSE;
	HKEY hKey;
...
BOOL ZOperateRegedit::SetREG_DWORDData(LPCTSTR lpSubKey, LPCTSTR lpValueName, const DWORD & dwData)
{
	BOOL nRtn = FALSE;
	HKEY hKey;
	if (RegCreateKey(HKEY_CURRENT_USER, lpSubKey, &hKey) == ERROR_SUCCESS)
...
BOOL ZOperateRegedit::GetREG_DWORDData(LPCTSTR lpSubKey, LPCTSTR lpValueName, DWORD & dwData)
{
	BOOL nRtn = FALSE;
	HKEY hKey;
	if (RegOpenKey(HKEY_CURRENT_USER, lpSubKey, &hKey) == ERROR_SUCCESS)
